...
window.OmegaConfig = {
  ...
  // Automatically detect environment: use Render URL for production, localhost for development
  ...
  CHAINGPT: {
    ...
    getApiKey: function () {
      ...
      if (this.PRODUCTION_API_KEY) {
        console.log(
          "[DEBUG] Using production ChainGPT API key from environment"
        );
    ...
    loadEnvVars: async function () {
      try {
        ...
        if (response.ok) {
          ...
          if (envData.CHAINGPT_API_KEY) {
            ...
            console.log(
              "[DEBUG] Loaded production API key from Vercel environment"
            );
            return true;
          }
        }
        ...
        console.log(
          "[DEBUG] Could not load environment variables from API:",
          error.message
        );
      }
      ...
      if (
        ...
        window.ENV &&
        window.ENV.CHAINGPT_API_KEY
      ) {
        this.PRODUCTION_API_KEY = window.ENV.CHAINGPT_API_KEY;
        console.log(
          "[DEBUG] Loaded production API key from build-time injection"
        );
        return true;
      }
...
console.log(
  "[DEBUG] ✅ Config loaded - RELAYER_URL:",
  window.OmegaConfig.RELAYER_URL
);
...
if (typeof window !== "undefined") {
  window.addEventListener("load", async () => {
    try {
      ...
      console.log(
        "[DEBUG] ⚠️ Could not load environment variables:",
        error.message
      );
    }
  });
