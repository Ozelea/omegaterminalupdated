...
(function () {
  ...
  window.OmegaCommands.Ethereum = {
    ...
    showCollections: async function (terminal, params) {
      ...
      try {
        const url = `https://terminal-v1-5-9.onrender.com/magiceden/eth/collections?limit=${limit}&sortBy=allTimeVolume`;
        ...
        terminal.logHtml(
          `<div style="font-size: 16px; font-weight: 500; color: #00D4FF; margin-bottom: 10px;">Top ${collections.length} Ethereum Collections</div>`,
          "output"
        );
        ...
        collections.forEach((collection, index) => {
          ...
          const imageUrl =
            collection.image || "https://via.placeholder.com/200?text=No+Image";

          // Volume and floor price formatting
          const volume = collection.volume?.allTime
            ? `${(collection.volume.allTime / 1e18).toFixed(2)} ETH`
            : "N/A";
          const floorPrice = collection.floorAsk?.price?.amount?.native
            ? `${collection.floorAsk.price.amount.native.toFixed(4)} ETH`
            : "N/A";
        ...
        if (
          ...
          terminal.log(
            "⚠️ Magic Eden API may be temporarily unavailable. Try again in a moment.",
            "warning"
          );
        }
      }
    ...
    showHelp: function (terminal) {
      ...
      terminal.log(
        "  eth collections [limit]    - Show top Ethereum collections",
        "output"
      );
      terminal.log("", "output");
      terminal.log("Examples:", "info");
      terminal.log(
        "  eth collections           - Show top 20 Ethereum collections",
        "output"
      );
      terminal.log(
        "  eth collections 10        - Show top 10 collections",
        "output"
      );
      terminal.log("", "output");
    },
