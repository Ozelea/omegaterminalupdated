...
(function() {
    ...
    window.MagicEdenAPI = {
        ...
        fetchStats: async function(collectionSymbol) {
            ...
            fetchListings: async function(collectionSymbol, limit = 20) {
                ...
                try {
                    ...
                    const data = await response.json();
                    console.log(`Magic Eden Listings Response (requested ${limit}, got ${data.length}):`, data);
                    return data;
                } catch (error) {
        ...
        formatSOL: function(price) {
            ...
            if (typeof price === 'object' && price.solPrice) {
                const sol = parseFloat(price.solPrice.rawAmount) / Math.pow(10, price.solPrice.decimals);
    ...
    function waitForTerminalME() {
        if (window.terminal && window.terminal.executeCommand && window.OmegaCommands && window.OmegaCommands.MagicEden) {
            console.log('🔮 Magic Eden Plugin: Terminal ready, integrating commands...');
    ...
    function initializeMagicEdenCommands() {
            ...
            if (cmd === 'magiceden' || cmd === 'me') {
                ...
                if (window.OmegaCommands && window.OmegaCommands.MagicEden) {
                    try {
                        ...
                        this.log('❌ Error executing Magic Eden command: ' + error.message, 'error');
                    }
                } else {
        ...
        console.log('✅ Magic Eden Plugin: Override active for "magiceden" and "me" commands');
    }
