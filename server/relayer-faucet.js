...
// ===================================
// PREDICTION MARKETS API PROXY (Polymarket & Kalshi)
// ===================================
...
// Function to sign Kalshi requests with RSA-PSS
function signKalshiRequest(message) {
  if (!KALSHI_PRIVATE_KEY || !KALSHI_API_KEY) {
    throw new Error("Kalshi credentials not configured in environment");
  }
  ...
  try {
    ...
    console.error("❌ Failed to sign Kalshi request:", error.message);
    throw error;
  }
}
...
async function withNetworkRetry(operation, context = "operation") {
  ...
  for (let attempt = 1; attempt <= NETWORK_RETRY_CONFIG.maxRetries; attempt++) {
    try {
      console.log(
        `[NETWORK] ${context} - Attempt ${attempt}/${NETWORK_RETRY_CONFIG.maxRetries}`
      );
      ...
      console.log(
        `[NETWORK] ${context} - Attempt ${attempt} failed: ${error.message}`
      );
      ...
      if (attempt < NETWORK_RETRY_CONFIG.maxRetries) {
        ...
        console.log(
          `[NETWORK] ${context} - Retrying in ${Math.round(delay)}ms...`
        );
  ...
  console.error(
    `[NETWORK] ${context} - All ${NETWORK_RETRY_CONFIG.maxRetries} attempts failed`
  );
  throw lastError;
}
...
async function getFreshNonce(walletAddress = null) {
  ...
  try {
    ...
    console.log(
      `[NONCE] 🔄 Fresh nonce for ${address.slice(0, 6)}...${address.slice(
        -4
      )}: ${nonce}`
    );
    return nonce;
    ...
    console.error(
      `[NONCE] ❌ Failed to get nonce for ${address}: ${error.message}`
    );
    throw error;
  }
}
...
async function initializeProvider() {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      // Only initialize wallet if RELAYER_PRIVATE_KEY is provided
      if (process.env.RELAYER_PRIVATE_KEY) {
        ...
        console.log(
          `[RELAYER] ⚠️  Running in proxy-only mode (no blockchain wallet)`
        );
      }
      return; // Success!
    } catch (error) {
  ...
  throw new Error(
    `RPC connection failed after 3 attempts: ${lastError.message}`
  );
}
...
async function fundMinerWalletIfNeeded(wallet) {
  try {
    ...
    if (balance.lt(ethers.utils.parseEther("0.0002"))) {
      ...
      await tx.wait();
      console.log(
        `Funded miner wallet ${wallet.address} with 0.001 OMEGA. Tx: ${tx.hash}`
      );
    }
  } catch (error) {
    console.error(
      `Failed to fund miner wallet ${wallet.address}:`,
      error.message
    );
  }
}
...
app.post("/fund", async (req, res) => {
  ...
  try {
    ...
    result
      ...
      .then((receipt) => {
        console.log(
          `[FUND] ✅ Confirmed: ${result.hash} - Block: ${receipt.blockNumber}`
        );
      })
      .catch((err) => {
        console.log(
          `[FUND] ⚠️  Confirmation failed: ${result.hash} - ${err.message}`
        );
      });
  } catch (error) {
    const responseTime = Date.now() - startTime;
    console.error(
      `[FUND] ❌ Failed after retries: ${error.message} (${responseTime}ms)`
    );
...
app.get("/dex/trending", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch trending tokens", success: false });
  }
});
...
app.get("/dex/boosted", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch boosted tokens", success: false });
  }
});
...
app.get("/dex/top-boosted", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch top boosted tokens", success: false });
  }
});
...
app.get("/dex/pair/:chainId/:pairId", async (req, res) => {
  try {
    ...
    if (data.pairs && Array.isArray(data.pairs)) {
      const enhancedData = {
        ...
        pairs: data.pairs.map((pair) => ({
          ...
          priceChangeFormatted: pair.priceChange?.h24
            ? `${pair.priceChange.h24.toFixed(2)}%`
            : "N/A",
          volumeFormatted: pair.volume?.h24
            ? `$${Number(pair.volume.h24).toLocaleString()}`
            : "N/A",
          liquidityFormatted: pair.liquidity?.usd
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch pair info", success: false });
  }
});
...
app.get("/dex/pools", async (req, res) => {
  try {
    const response = await fetch(
      "https://api.dexscreener.com/latest/dex/pools"
    );
    const data = await response.json();
...
app.get("/dex/tokens/:chainId/:tokenAddresses", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch multiple tokens", success: false });
  }
});
...
app.get("/dex/orders/:chainId/:tokenAddress", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch token orders", success: false });
  }
});
...
app.get("/gecko/search", async (req, res) => {
  try {
    const q = req.query.q;
    const response = await fetch(
      `https://api.geckoterminal.com/api/v2/search/pairs?query=${encodeURIComponent(
        q
      )}`,
...
app.get("/gecko/networks", async (req, res) => {
  try {
    ...
    const response = await fetch(`${GECKO_API}/networks${page}`, {
...
app.get("/gecko/networks/:network/dexes", async (req, res) => {
  try {
    ...
    const response = await fetch(`${GECKO_API}/networks/${network}/dexes`, {
      headers: GECKO_HEADERS,
    });
    const data = await response.json();
    res.json(data);
...
app.get("/gecko/networks/:network/pools", async (req, res) => {
  try {
    const { network } = req.params;
    const response = await fetch(`${GECKO_API}/networks/${network}/pools`, {
      headers: GECKO_HEADERS,
    });
    const data = await response.json();
    res.json(data);
...
app.get(
  "/gecko/networks/:network/tokens/:token_address/pools",
  async (req, res) => {
    try {
      const { network, token_address } = req.params;
      const response = await fetch(
        `${GECKO_API}/networks/${network}/tokens/${token_address}/pools`,
        { headers: GECKO_HEADERS }
      );
      const data = await response.json();
      res.json(data);
...
app.get(
  "/gecko/networks/:network/pools/:pool_address/info",
  async (req, res) => {
    try {
      const { network, pool_address } = req.params;
      const response = await fetch(
        `${GECKO_API}/networks/${network}/pools/${pool_address}/info`,
        { headers: GECKO_HEADERS }
      );
      const data = await response.json();
      res.json(data);
...
app.get(
  "/gecko/networks/:network/pools/:pool_address/ohlcv/:timeframe",
  async (req, res) => {
    try {
      const { network, pool_address, timeframe } = req.params;
      const response = await fetch(
        `${GECKO_API}/networks/${network}/pools/${pool_address}/ohlcv/${timeframe}`,
        { headers: GECKO_HEADERS }
      );
      const data = await response.json();
      res.json(data);
...
app.get(
  "/gecko/networks/:network/pools/:pool_address/trades",
  async (req, res) => {
    try {
      const { network, pool_address } = req.params;
      const params = new URLSearchParams(req.query).toString();
      const url = `${GECKO_API}/networks/${network}/pools/${pool_address}/trades${
        params ? "?" + params : ""
      }`;
...
app.get("/stock/quote/:symbol", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Alpha Vantage error", details: err.message });
  }
});
  ...
  try {
    ...
    const result = await withNetworkRetry(async () => {
      ...
      do {
        ...
        attempts++;
        if (
          !pendingTxs[wallet.address] &&
          (!busyWallets[wallet.address] || busyWallets[wallet.address] < now)
        )
          break;
      ...
      if (
        ...
        throw new Error(
          "All mining wallets are busy, please try again in a moment."
        );
      }
      ...
      console.log(
        `[MINE] 🔄 Using fresh nonce for miner ${wallet.address.slice(
          0,
          6
        )}...${wallet.address.slice(-4)}: ${nonce}`
      );
    ...
    if (rand < 0.95) {
      // 95% chance for rewards!
      if (rand < 0.1)
        reward = parseFloat((Math.random() * 0.5 + 0.1).toFixed(4));
      // 10% chance for 0.1-0.6 OMEGA (big rewards)
      else if (rand < 0.3)
        reward = parseFloat((Math.random() * 0.08 + 0.02).toFixed(4));
      // 20% chance for 0.02-0.1 OMEGA (good rewards)
      else if (rand < 0.6)
        reward = parseFloat((Math.random() * 0.02 + 0.005).toFixed(4));
      // 30% chance for 0.005-0.025 OMEGA (medium rewards)
    ...
    result.tx
      ...
      .then(() => {
        ...
        console.log(
          `[MINE] ${userAddr} reward: ${reward}, total: ${rewardsByAddress[userAddr]}`
        );
      })
      .finally(() => {
...
app.post("/jupiter/quote", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (data.error || data.message) {
        res.status(400).json({
          error:
            data.error ||
            data.message ||
            "No swap route found for this pair and amount",
        });
      } else {
        res.json(data);
      }
      ...
      res
        .status(400)
        .json({ error: "No swap route found for this pair and amount" });
    }
  } catch (err) {
    res
      .status(500)
      .json({ error: "Failed to fetch Jupiter quote", details: err.message });
  }
});
...
app.post("/jupiter/swap", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: "Failed to parse Jupiter quote response" });
    }
    ...
    try {
      ...
      return res
        .status(400)
        .json({ error: "Failed to parse Jupiter swap response" });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to create Jupiter swap", details: err.message });
  }
});
    ...
    db.get(
      ...
      (err, profile) => {
        ...
        if (!profile) {
          return res
            .status(404)
            .json({ error: "User profile not found", status: 404 });
        }

        // Get discover interactions (activities)
        db.all(
          `SELECT * FROM user_activities WHERE wallet_address = ? ORDER BY timestamp DESC LIMIT 10`,
          [walletAddress],
          (err, activities) => {
            if (err) activities = [];
...
app.post("/v1/users/:walletAddress/profile", async (req, res) => {
  try {
    ...
    db.run(
      ...
      function (err) {
        if (err) {
          return res
            .status(500)
            .json({ error: "Failed to update profile", status: 500 });
        }
    ...
    res
      .status(500)
      .json({ error: "Failed to update user profile", status: 500 });
  }
});
...
app.get("/v1/users/:walletAddress/activity", async (req, res) => {
  try {
    ...
    db.all(
      `SELECT * FROM user_activities WHERE wallet_address = ? 
...
app.get("/v1/users/:walletAddress/tasks", async (req, res) => {
  try {
    ...
    db.all(
      `SELECT * FROM user_tasks WHERE wallet_address = ? ORDER BY created_at DESC`,
      [walletAddress],
      (err, tasks) => {
        if (err) {
          return res.status(500).json({ error: "Database error", status: 500 });
...
app.get("/v1/users/:walletAddress/faucet-activities", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get faucet activities", status: 500 });
  }
});
...
app.get("/v1/users/:walletAddress/blockchain-claims", async (req, res) => {
  try {
    ...
    db.all(
      `SELECT * FROM faucet_claims WHERE wallet_address = ? AND status = 'CONFIRMED' 
    ...
    res
      .status(500)
      .json({ error: "Failed to get blockchain claims", status: 500 });
  }
});
...
app.get("/v1/users/:walletAddress/aggregated-claims", async (req, res) => {
  try {
    ...
    db.all(
      `SELECT * FROM faucet_claims WHERE wallet_address = ?`,
      [walletAddress],
      (err, claims) => {
        if (err) {
          return res.status(500).json({ error: "Database error", status: 500 });
    ...
    res
      .status(500)
      .json({ error: "Failed to get aggregated claims", status: 500 });
  }
});

// Get User Aggregated Achievements
app.get(
  "/v1/users/:walletAddress/aggregated-achievements",
  async (req, res) => {
    try {
      const { walletAddress } = req.params;

      db.all(
        `SELECT ua.*, a.title, a.points FROM user_achievements ua 
        ...
        (err, achievements) => {
          if (err) {
            return res
              .status(500)
              .json({ error: "Database error", status: 500 });
          }
      ...
      res
        .status(500)
        .json({ error: "Failed to get aggregated achievements", status: 500 });
    }
  }
);
...
app.post("/v1/referrals/generate-code", async (req, res) => {
  try {
    ...
    db.get(
      `SELECT referral_code FROM referrals WHERE referrer_address = ?`,
      [walletAddress],
      (err, existing) => {
        if (err) {
          return res.status(500).json({ error: "Database error", status: 500 });
        ...
        const referralCode =
          "OMEGA" + Math.random().toString(36).substring(2, 8).toUpperCase();

        db.run(
          `INSERT INTO referrals (referral_code, referrer_address) VALUES (?, ?)`,
    ...
    res
      .status(500)
      .json({ error: "Failed to generate referral code", status: 500 });
  }
});
...
app.get("/v1/referrals/validate/:code", async (req, res) => {
  try {
    ...
    db.get(
      `SELECT r.*, p.username, p.display_name 
                FROM referrals r 
    ...
    res
      .status(500)
      .json({ error: "Failed to validate referral code", status: 500 });
  }
});
...
app.post("/v1/referrals/complete", async (req, res) => {
  try {
    ...
    if (!referralCode || !referredAddress) {
      return res.status(400).json({
        error: "Referral code and referred address required",
        status: 400,
      });
    }
...
app.post("/v1/referrals/track-visit", async (req, res) => {
  try {
    ...
    console.log(
      `[REFERRAL] Visit tracked: ${referralCode} by ${
        visitorAddress || "anonymous"
      }`
    );
    ...
    if (visitorAddress) {
      db.run(
        `INSERT INTO user_activities (wallet_address, activity_type, details) 
                    VALUES (?, 'referral_visit', ?)`,
        [
          visitorAddress,
          JSON.stringify({ referralCode, timestamp: new Date() }),
        ]
      );
    }
...
app.get("/v1/ambassadors/leaderboard", async (req, res) => {
  try {
    ...
    db.all(
      `SELECT p.wallet_address, p.username, p.display_name, pr.total_points, pr.level, 
        ...
        db.get(
          ...
          (err, countResult) => {
            ...
            const formattedAmbassadors = ambassadors.map((amb, index) => ({
              ...
              username:
                amb.username || `user_${amb.wallet_address.substring(2, 8)}`,
    ...
    res
      .status(500)
      .json({ error: "Failed to get ambassador leaderboard", status: 500 });
  }
});
...
app.get("/v1/ambassadors/:address/profile", async (req, res) => {
  try {
    ...
    db.get(
      `SELECT p.*, pr.total_points, pr.level, pr.rank, pr.referrals_count
                FROM user_profiles p
                LEFT JOIN user_progress pr ON p.wallet_address = pr.wallet_address
      ...
      (err, profile) => {
        ...
        if (!profile) {
          return res
            .status(404)
            .json({ error: "Ambassador not found", status: 404 });
        }
    ...
    res
      .status(500)
      .json({ error: "Failed to get ambassador profile", status: 500 });
  }
});
...
app.get("/v1/ambassadors/:address/referrals", async (req, res) => {
  try {
    ...
    db.all(
      `SELECT * FROM referrals WHERE referrer_address = ? ORDER BY created_at DESC`,
      [address],
      (err, referrals) => {
        if (err) {
          return res.status(500).json({ error: "Database error", status: 500 });
    ...
    res
      .status(500)
      .json({ error: "Failed to get ambassador referrals", status: 500 });
  }
});
...
app.get("/v1/ambassadors/:address/stats", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get ambassador stats", status: 500 });
  }
});
  ...
  try {
    ...
    db.get(
      ...
      (err, existing) => {
        ...
        const referralCode =
          "AMB" + Math.random().toString(36).substring(2, 10).toUpperCase();
        ...
        db.run(
          ...
          function (err) {
            if (err) {
              return res
                .status(500)
                .json({ error: "Failed to generate code", status: 500 });
            }
    ...
    res.status(500).json({
      error: "Failed to generate ambassador referral code",
      status: 500,
    });
  }
});
...
app.get("/v1/ambassadors/directory", async (req, res) => {
  try {
    db.all(
      `SELECT p.wallet_address, p.username, p.display_name, p.bio, p.avatar,
    ...
    res
      .status(500)
      .json({ error: "Failed to get ambassador directory", status: 500 });
  }
});
...
app.get("/v1/arcade/leaderboard", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to get arcade leaderboard", status: 500 });
  }
});
...
app.post("/api/achievements/track", async (req, res) => {
  try {
    ...
    if (!walletAddress || !achievementId) {
      return res.status(400).json({
        error: "Wallet address and achievement ID required",
        status: 400,
      });
    }
    ...
    db.get(
      `SELECT * FROM user_achievements WHERE wallet_address = ? AND achievement_id = ?`,
      ...
      (err, existing) => {
        ...
        // Award achievement
        db.run(
          `INSERT INTO user_achievements (wallet_address, achievement_id, details) 
                    VALUES (?, ?, ?)`,
          [walletAddress, achievementId, JSON.stringify(details)],
          function (err) {
            if (err) {
              return res
                .status(500)
                .json({ error: "Failed to track achievement", status: 500 });
            }
            ...
            db.run(
              ...
              [
                walletAddress,
                JSON.stringify({ achievementId, action, details }),
              ]
            );
...
app.post("/api/achievements/track-social", async (req, res) => {
  try {
    ...
    db.run(
      `INSERT OR IGNORE INTO user_achievements (wallet_address, achievement_id, details) 
                VALUES (?, ?, ?)`,
      [
        walletAddress,
        achievementId,
        JSON.stringify({ platform, action, details }),
      ],
      function (err) {
        if (err) {
          return res
            .status(500)
            .json({ error: "Failed to track social achievement", status: 500 });
        }
  ...
  try {
    ...
    db.run(
      ...
      [
        referrerAddress,
        referrerAchievement,
        JSON.stringify({ referralCode, referredAddress }),
      ]
    );
    ...
    db.run(
      ...
      [
        referredAddress,
        referredAchievement,
        JSON.stringify({ referralCode, referrerAddress }),
      ]
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to award referral achievements", status: 500 });
  }
});
...
app.post("/v1/wallet-verification", async (req, res) => {
  try {
    ...
    if (!walletAddress || !message || !signature) {
      return res.status(400).json({
        error: "Wallet address, message, and signature required",
        status: 400,
      });
    }
...
app.post("/api/popup-submission", async (req, res) => {
  try {
    ...
    db.run(
      ...
      [
        submissionData.walletAddress || "anonymous",
      ...
      function (err) {
        if (err) {
          return res
            .status(500)
            .json({ error: "Failed to store submission", status: 500 });
        }
    ...
    res
      .status(500)
      .json({ error: "Failed to process popup submission", status: 500 });
  }
});
...
app.post("/api/reset-progress", async (req, res) => {
  try {
    ...
    // Reset user progress
    db.run(`DELETE FROM user_progress WHERE wallet_address = ?`, [
      walletAddress,
    ]);
    db.run(`DELETE FROM user_achievements WHERE wallet_address = ?`, [
      walletAddress,
    ]);
    db.run(`DELETE FROM user_activities WHERE wallet_address = ?`, [
      walletAddress,
    ]);
    ...
    db.run(
      ...
      function (err) {
        if (err) {
          return res
            .status(500)
            .json({ error: "Failed to cleanup activities", status: 500 });
        }
...
app.get("/", (req, res) => {
  res.json({
    ...
    endpoints: {
      ...
      arcade: ["/v1/arcade/leaderboard"],
      faucet: ["/api/faucet/status", "/api/faucet/claim", "/api/faucet/claims"],
      achievements: [
...
async function startServer() {
  console.log(
    "🚀 INITIALIZING NETWORK-RESILIENT RELAYER (COMPLETE VERSION)..."
  );
  console.log("🔧 Network retry enabled for ETIMEDOUT/ENETUNREACH errors");
  console.log(
    "🔧 Fresh nonce management enabled for all blockchain transactions"
  );

  if (!process.env.RELAYER_PRIVATE_KEY) {
    console.warn(
      "⚠️  WARNING: RELAYER_PRIVATE_KEY not found - blockchain operations will be disabled"
    );
    console.warn("⚠️  Proxy endpoints (Polymarket, Kalshi) will still work");
    // Don't exit - allow proxy-only mode
  }
  ...
  app.use("/kalshi", async (req, res) => {
    const kalshiPath = req.path;
    ...
    // Automatically add authentication headers if credentials are available
    if (KALSHI_API_KEY && KALSHI_PRIVATE_KEY) {
      try {
        const timestamp = Date.now().toString();
        const method = req.method;

        // Strip query parameters from path for signing (per Kalshi docs)
        const pathWithoutQuery = kalshiPath.split("?")[0];

        // Create message to sign: timestamp + method + path
        const messageToSign = timestamp + method + pathWithoutQuery;

        // Sign the message
        const signature = signKalshiRequest(messageToSign);
        ...
        console.log(`🔑 Added server-side Kalshi authentication`);
      } catch (error) {
        console.error(`❌ Failed to add authentication: ${error.message}`);
        // Continue without auth - will work for public endpoints
      }
    }

    try {
      // Use Kalshi Demo API (production endpoint not publicly accessible)
      // Demo environment is publicly available at demo-api.kalshi.co
      const kalshiUrl = `https://demo-api.kalshi.co/trade-api/v2${kalshiPath}`;
      ...
      if (error.response) {
        ...
        res.status(500).json({
          ...
          message: error.message,
          details:
            "Could not reach Kalshi API. Check your internet connection or try: kalshi status",
        });
      }
    }
  });
  ...
  const server = app.listen(PORT, () => {
    console.log(`🚀 OMEGA UNIFIED SERVER STARTED ON PORT ${PORT}`);
    console.log(`⏰ Started at: ${new Date().toISOString()}`);
    console.log(
      `📊 Polymarket endpoint: http://localhost:${PORT}/polymarket/*`
    );
    console.log(`🎯 Kalshi endpoint: http://localhost:${PORT}/kalshi/*`);
    console.log(
      KALSHI_API_KEY
        ? "🔑 Kalshi authentication enabled"
        : "⚠️  Kalshi auth disabled (no credentials in .env)"
    );
  ...
  try {
    ...
    console.log(
      `📡 API Endpoints: Gemini AI, DexScreener, GeckoTerminal, Alpha Vantage, Jupiter`
    );
  } catch (error) {
    console.error(
      "⚠️  RPC initialization failed, but server is running:",
      error.message
    );
    console.error(
      "🔄 Endpoints will retry connections automatically when called"
    );
...
app.get("/ref/tokens", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch REF tokens", success: false });
  }
});
  ...
  try {
    ...
    if (data && Array.isArray(data)) {
      const transformedData = data.map((pool) => ({
        ...
        tvlFormatted: pool.tvl
          ? `$${Number(pool.tvl).toLocaleString()}`
          : "N/A",
        volumeFormatted: pool.volume_24h
          ? `$${Number(pool.volume_24h).toLocaleString()}`
          : "N/A",
      }));
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch REF pools", success: false });
  }
});
...
app.get("/ref/pool/:poolId", async (req, res) => {
  try {
    ...
    const response = await fetch(
      `${REF_CONFIG.INDEXER_URL}/get-pool?pool_id=${poolId}`
    );
    const data = await response.json();
    ...
    if (data) {
      const enhancedData = {
        ...
        data: {
          ...
          tvlFormatted: data.tvl
            ? `$${Number(data.tvl).toLocaleString()}`
            : "N/A",
          volumeFormatted: data.volume_24h
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch pool details", success: false });
  }
});
...
app.get("/ref/orders/:accountId", async (req, res) => {
  try {
    ...
    const activeResponse = await fetch(
      `${REF_CONFIG.INDEXER_URL}/list-active-orders?account_id=${accountId}`
    );
    const activeOrders = await activeResponse.json();

    // Get order history
    const historyResponse = await fetch(
      `${REF_CONFIG.INDEXER_URL}/list-history-orders?account_id=${accountId}`
    );
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch REF orders", success: false });
  }
});
...
app.get("/ref/balances/:accountId", async (req, res) => {
  try {
    const { accountId } = req.params;
    const response = await fetch(
      `${REF_CONFIG.INDEXER_URL}/list-user-assets?account_id=${accountId}`
    );
    const data = await response.json();
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch REF balances", success: false });
  }
});
...
app.get("/ref/search", async (req, res) => {
  try {
    ...
    if (!query) {
      return res
        .status(400)
        .json({ error: "Search query required", success: false });
    }
    ...
    res
      .status(500)
      .json({ error: "Failed to search REF tokens", success: false });
  }
});
  ...
  try {
    ...
    if (pools && Array.isArray(pools)) {
      ...
      const totalVolume24h = pools.reduce(
        (sum, pool) => sum + (parseFloat(pool.volume_24h) || 0),
        0
      );
      const activePools = pools.filter(
        (pool) => pool.tvl && parseFloat(pool.tvl) > 1000
      ).length;
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch REF stats", success: false });
  }
});
...
app.post("/api/bots/start", async (req, res) => {
  try {
    ...
    if (!scriptName || !scriptPath) {
      return res
        .status(400)
        .json({ error: "Script name and path required", success: false });
    }
    ...
    if (runningBots.has(scriptName)) {
      return res
        .status(400)
        .json({ error: "Bot is already running", success: false });
    }
    ...
    if (!fs.existsSync(scriptPath)) {
      return res
        .status(404)
        .json({ error: "Script file not found", success: false });
    }
    ...
    pythonProcess.on("exit", (code, signal) => {
      ...
      if (bot) {
        ...
        buffer.push({
          timestamp: Date.now(),
          type: "system",
          content: `Bot exited with code ${code}${
...
app.post("/api/bots/stop", async (req, res) => {
  try {
    ...
    if (!scriptName) {
      return res
        .status(400)
        .json({ error: "Script name required", success: false });
    }
    ...
    if (!bot) {
      return res
        .status(404)
        .json({ error: "Bot not found or not running", success: false });
    }
    ...
    res.json({
      ...
      data: {
        ...
        performance: {
          ...
          runtime:
            performance.runtime || (bot ? Date.now() - bot.startTime : 0),
        },
      },
    });
  } catch (error) {
...
app.post("/api/bots/:scriptName/input", async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Failed to send input to bot",
      details: error.message,
      success: false,
    });
  }
...
app.post("/api/bots/upload", async (req, res) => {
  try {
    ...
    if (!scriptName || !scriptContent) {
      return res
        .status(400)
        .json({ error: "Script name and content required", success: false });
    }
