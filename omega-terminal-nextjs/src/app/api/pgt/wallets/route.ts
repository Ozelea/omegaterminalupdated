/**
 * PGT Wallets API Route
 * Server-side proxy for PGT tracked wallets list with caching
 */
...
export async function GET(request: NextRequest) {
  try {
    const response = await fetch(`${PGT_BASE_URL}/wallets`, {
      headers: {
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch wallets from PGT",
      },
      { status: 500 }
    );
  }
