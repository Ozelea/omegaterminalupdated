/**
 * PGT Portfolio API Route
 * Server-side proxy for PGT portfolio data with caching
 */
...
export async function GET(request: NextRequest) {
  try {
    const response = await fetch(`${PGT_BASE_URL}/portfolio`, {
      headers: {
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Failed to fetch portfolio from PGT",
      },
      { status: 500 }
    );
  }
