...
(function () {
  ...
  function waitForTerminal() {
    ...
    if (window.terminal && window.terminal.logHtml && window.terminal.log) {
      ...
      if (
        typeof window.terminal.handleCommand === "function" ||
        initAttempts > 10
      ) {
        initializeUltimateAnalytics();
      } else {
        console.log(
          "⏳ Terminal functions still loading, waiting...",
          initAttempts
        );
      ...
      console.error(
        "❌ DexScreener Analytics: Failed to initialize after maximum attempts"
      );
    }
  }
  ...
  function initializeUltimateAnalytics() {
    console.log(
      "✅ DexScreener Analytics Ultimate: Terminal detected, initializing..."
    );
    ...
    if (window.terminal.handleDexScreenerCommand) {
      window.terminal._originalHandleDexScreenerCommand =
        window.terminal.handleDexScreenerCommand;
    ...
    window.terminal.handleDexScreenerCommand = async function (args) {
      ...
      try {
        switch (sub) {
          ...
          default:
            this.log(
              `❌ Unknown DexScreener Analytics command: ${sub}`,
              "error"
            );
            this.displayUltimateHelp();
        }
    ...
    window.terminal.displayUltimateHelp = function () {
      ...
      this.log(
        "  dexscreener search <token>                Search tokens with rich analytics",
        "output"
      );
      this.log(
        "  dexscreener filter --help                 Advanced filtering options",
        "output"
      );
      this.log(
        "  dexscreener scan trending                 Smart scan trending tokens",
        "output"
      );
      this.log(
        "  dexscreener discover newgems              Auto-discover new gems",
        "output"
      );
      this.log("", "info");
      ...
      this.log(
        "  dexscreener analytics <token>             Full token analytics dashboard",
        "output"
      );
      this.log(
        "  dexscreener compare <token1> <token2>     Side-by-side token comparison",
        "output"
      );
      this.log("", "info");
      ...
      this.log(
        "  dexscreener portfolio                     View your tracked tokens",
        "output"
      );
      this.log(
        "  dexscreener watchlist add <token>         Add token to watchlist",
        "output"
      );
      this.log(
        "  dexscreener watchlist view                View watchlist",
        "output"
      );
      this.log("", "info");
      ...
      this.log(
        "  dexscreener alerts create                 Create price/volume alerts",
        "output"
      );
      this.log(
        "  dexscreener monitor start <token>         Start monitoring token",
        "output"
      );
      this.log("", "info");
      ...
      this.log(
        "  dexscreener newgems                       Find new token opportunities",
        "info"
      );
      this.log(
        "  dexscreener trending --volume-min=1M      Filter trending by volume",
        "info"
      );
      this.log(
        "  dexscreener filter --chain=solana         Filter by specific blockchain",
        "info"
      );
      this.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "info");
    };
    ...
    window.terminal.handleUltimateSearch = async function (args) {
      if (!args[2]) {
        ...
        this.log(
          "  --volume-min=1000000       Minimum 24h volume (USD)",
          "info"
        );
      ...
      try {
        ...
        if (data.pairs && data.pairs.length > 0) {
          ...
          this.log(
            `✅ Found ${filteredPairs.length} tokens matching your search:`,
            "success"
          );
          this.log("", "info");
    ...
    window.terminal.handleTokenAnalytics = async function (args) {
      if (!args[2]) {
        this.log(
          "Usage: dexscreener analytics <token_address_or_symbol>",
          "info"
        );
        return;
      }
    ...
    window.terminal.handleAdvancedFilter = async function (args) {
      ...
      try {
        ...
        this.log(
          `✅ Found ${results.length} tokens matching your filters:`,
          "success"
        );
        this.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━", "info");
    ...
    window.terminal.handlePortfolio = async function (args) {
      ...
      switch (subCommand) {
        ...
        default:
          this.log(
            "Usage: dexscreener portfolio [add|remove|view|clear] [token]",
            "info"
          );
      }
    };
    ...
    window.terminal.handleWatchlist = async function (args) {
      ...
      switch (subCommand) {
        ...
        default:
          this.log(
            "Usage: dexscreener watchlist [add|remove|view|clear] [token]",
            "info"
          );
      }
    };
    ...
    window.terminal.handleAlerts = async function (args) {
      ...
      switch (subCommand) {
        ...
        default:
          this.log(
            "Usage: dexscreener alerts [create|list|remove|clear]",
            "info"
          );
      }
    };
    ...
    window.terminal.applySearchFilters = function (pairs, filters) {
      return pairs.filter((pair) => {
        ...
        if (
          filters.volumeMin &&
          (!pair.volume?.h24 || pair.volume.h24 < filters.volumeMin)
        ) {
          return false;
        }
        ...
        if (
          ...
          (!pair.marketCap || pair.marketCap < filters.marketCapMin)
        ) {
          return false;
        }
        ...
        if (filters.ageMax && pair.pairCreatedAt) {
          const ageInDays = Math.floor(
            (Date.now() - pair.pairCreatedAt * 1000) / (1000 * 60 * 60 * 24)
          );
    ...
    window.terminal.displayEnhancedTokenCard = function (pair, index) {
      ...
      if (pair.info?.imageUrl) {
        ...
        html += `<div style='width:36px;height:36px;border-radius:50%;background:#00ff80;color:#000;display:flex;align-items:center;justify-content:center;font-weight:bold;'>${symbol.charAt(
          0
        )}</div>`;
      }

      html += `<div>`;
      html += `<div style='font-size:1.4em;font-weight:bold;color:#00ff80;'>${
        pair.baseToken?.symbol || "N/A"
      }</div>`;
      html += `<div style='color:#888;font-size:0.9em;'>${
        pair.baseToken?.name || "Unknown Token"
      }</div>`;
      html += `<div style='color:#666;font-size:0.8em;'>${pair.chainId.toUpperCase()} • ${
      ...
      if (pair.priceChange?.h24 !== undefined) {
        ...
        html += `<div style='color:${changeColor};font-size:1em;font-weight:bold;'>${changePrefix}${pair.priceChange.h24.toFixed(
          2
        )}% (24h)</div>`;
      }
      html += `</div></div>`;
      ...
      if (pair.volume?.h24) {
        ...
        html += `<div style='color:#fff;font-weight:bold;'>$${this.formatNumber(
          pair.volume.h24
        )}</div>`;
        html += `</div>`;
      } else {
      ...
      if (pair.txns?.h24) {
        ...
        const buyPercentage =
          pair.txns.h24.buys && totalTxns > 0
            ? ((pair.txns.h24.buys / totalTxns) * 100).toFixed(0)
            : 0;
      ...
      if (pair.pairCreatedAt) {
        ...
        const ageColor =
          ageInDays <= 7 ? "#00ff00" : ageInDays <= 30 ? "#ffcc00" : "#888";
      ...
      html += `<button onclick="window.terminal.showTokenAnalytics('${
        pair.baseToken.address || pair.pairAddress
      }', '${
        pair.baseToken.symbol || "TOKEN"
      }')" style='background:linear-gradient(135deg,#ff6b6b,#ee5a52);color:#fff;border:none;padding:8px 14px;border-radius:4px;cursor:pointer;font-size:0.85em;font-weight:bold;box-shadow:0 2px 6px rgba(255,107,107,0.3);'>📊 Analytics</button>`;
    ...
    window.terminal.showTokenAnalytics = async function (
      tokenAddress,
      tokenSymbol
    ) {
      ...
      try {
        ...
        this.log(
          `❌ Analytics failed for ${tokenSymbol}: ${error.message}`,
          "error"
        );
      }
    };
    ...
    window.terminal.displayAnalyticsDashboard = function (pair) {
      ...
      if (pair.info?.imageUrl) {
        ...
        const symbol = pair.baseToken?.symbol || "T";
        html += `<div style='width:64px;height:64px;border-radius:50%;background:#00ff80;color:#000;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:2em;border:3px solid #00ff80;box-shadow:0 4px 12px rgba(0,255,128,0.4);'>${symbol.charAt(
          0
        )}</div>`;
      }

      html += `<div>`;
      html += `<div style='font-size:2.2em;font-weight:bold;color:#00ff80;text-shadow:0 2px 4px rgba(0,255,128,0.3);'>${
        pair.baseToken?.symbol || "N/A"
      }</div>`;
      html += `<div style='color:#bbb;font-size:1.2em;margin:4px 0;'>${
        pair.baseToken?.name || "Unknown Token"
      }</div>`;
      html += `<div style='color:#888;font-size:1em;'>${pair.chainId.toUpperCase()} • ${
        pair.dexId || "Unknown DEX"
      }</div>`;
      html += `</div></div>`;
      ...
      if (pair.priceChange?.h24 !== undefined) {
        const changeColor = pair.priceChange.h24 >= 0 ? "#00ff00" : "#ff3333";
        const changePrefix = pair.priceChange.h24 >= 0 ? "+" : "";
        html += `<div style='color:${changeColor};font-size:1.4em;font-weight:bold;text-shadow:0 2px 4px rgba(${
          pair.priceChange.h24 >= 0 ? "0,255,0" : "255,51,51"
        },0.3);margin-top:8px;'>${changePrefix}${pair.priceChange.h24.toFixed(
          2
        )}% (24h)</div>`;
      }
      html += `</div></div>`;
      ...
      if (pair.volume?.h24) {
        html += `<div style='color:#fff;font-weight:bold;font-size:1.3em;'>$${this.formatNumber(
          pair.volume.h24
        )}</div>`;
      ...
      if (pair.volume?.h1) {
        html += `<div style='color:#bbb;font-size:0.85em;margin-top:8px;border-top:1px solid rgba(255,204,0,0.2);padding-top:8px;'>1h: $${this.formatNumber(
          pair.volume.h1
        )}</div>`;
      }
      html += `</div>`;
      ...
      if (pair.liquidity?.base && pair.liquidity?.quote) {
        html += `<div style='color:#bbb;font-size:0.8em;margin-top:8px;border-top:1px solid rgba(0,204,255,0.2);padding-top:8px;'>Base: ${this.formatNumber(
          pair.liquidity.base
        )}<br>Quote: ${this.formatNumber(pair.liquidity.quote)}</div>`;
      }
      html += `</div>`;
      ...
      if (pair.marketCap || pair.fdv) {
        ...
        html += `<div style='color:#ff6666;font-size:0.9em;margin-top:4px;'>${
          pair.marketCap ? "Market Cap" : "FDV"
        }</div>`;
      } else {
        html += `<div style='color:#888;font-size:1.1em;'>No data available</div>`;
      }
      ...
      if (pair.txns?.h24) {
        ...
        const buyPercentage =
          pair.txns.h24.buys && totalTxns > 0
            ? ((pair.txns.h24.buys / totalTxns) * 100).toFixed(1)
            : 0;
        ...
        html += `<div style='color:#00ff00;'>Buys: ${
          pair.txns.h24.buys || 0
        } (${buyPercentage}%)</div>`;
        html += `<div style='color:#ff6666;'>Sells: ${
          pair.txns.h24.sells || 0
        } (${(100 - buyPercentage).toFixed(1)}%)</div>`;
        html += `</div>`;
      } else {
        html += `<div style='color:#888;font-size:1.1em;'>No data available</div>`;
      }
      ...
      ["5m", "1h", "6h", "24h"].forEach((timeframe) => {
        if (pair.priceChange && pair.priceChange[timeframe] !== undefined) {
          ...
          html += `<div style='padding:8px;background:rgba(${
            change >= 0 ? "0,255,0" : "255,51,51"
          },0.1);border-radius:6px;border:1px solid ${changeColor};'>`;
          html += `<div style='color:#bbb;font-size:0.8em;'>${timeframe.toUpperCase()}</div>`;
          html += `<div style='color:${changeColor};font-weight:bold;font-size:1.1em;'>${changePrefix}${change.toFixed(
            2
          )}%</div>`;
          html += `</div>`;
        }
      ...
      if (pair.pairCreatedAt || pair.boosts?.active) {
        ...
        if (pair.pairCreatedAt) {
          ...
          const ageColor =
            ageInDays <= 7 ? "#00ff00" : ageInDays <= 30 ? "#ffcc00" : "#888";
          html += `<div style='padding:12px;background:rgba(${
            ageInDays <= 7
              ? "0,255,0"
              : ageInDays <= 30
              ? "255,204,0"
              : "136,136,136"
          },0.1);border-radius:6px;'>`;
      ...
      html += `<button onclick="window.terminal.addToWatchlist('${
        pair.baseToken.address || pair.pairAddress
      }')" style='background:linear-gradient(135deg,#00ff80,#00cc66);color:#000;border:none;padding:10px 16px;border-radius:6px;cursor:pointer;font-weight:bold;box-shadow:0 4px 12px rgba(0,255,128,0.3);transition:all 0.3s;'>📋 Add to Watchlist</button>`;
      html += `<button onclick="window.terminal.addToPortfolio('${
        pair.baseToken.address || pair.pairAddress
      }')" style='background:linear-gradient(135deg,#ff8800,#ff6600);color:#fff;border:none;padding:10px 16px;border-radius:6px;cursor:pointer;font-weight:bold;box-shadow:0 4px 12px rgba(255,136,0,0.3);transition:all 0.3s;'>💼 Add to Portfolio</button>`;
      ...
      if (
        pair.info?.socials &&
        Array.isArray(pair.info.socials) &&
          ...
          if (social.type && social.url) {
            ...
            switch (social.platform.toLowerCase()) {
              ...
              case "telegram":
                ...
                break;
              case "discord":
                socialUrl = social.handle.startsWith("http")
                  ? social.handle
      ...
      if (
        pair.info?.websites &&
        Array.isArray(pair.info.websites) &&
    ...
    window.terminal.viewPortfolio = async function () {
      try {
        ...
        if (portfolio.length === 0) {
          ...
          this.log(
            'Use "dexscreener portfolio add <token>" to add tokens',
            "info"
          );
          return;
        }
        ...
        for (let i = 0; i < portfolio.length; i++) {
          ...
          try {
            ...
            if (data.pairs && data.pairs.length > 0) {
              ...
              html += `</div>`;
              html += `<div style='text-align:right;'>`;
              html += `<div style='color:#fff;font-weight:bold;'>$${currentPrice.toFixed(
                6
              )}</div>`;
              html += `<div style='color:${changeColor};font-size:0.9em;'>${changePrefix}${priceChange.toFixed(
                2
              )}%</div>`;
              html += `</div>`;
              html += `</div>`;
              html += `<div style='font-size:0.8em;color:#666;margin-top:8px;'>Added: ${new Date(
                item.addedAt
              ).toLocaleDateString()} at $${addedPrice.toFixed(6)}</div>`;
              html += `</div>`;
            ...
            this.log(
              `⚠️ Failed to update ${item.symbol}: ${error.message}`,
              "error"
            );
          }
        }
    ...
    window.terminal.viewWatchlist = async function () {
      try {
        ...
        if (watchlist.length === 0) {
          ...
          this.log(
            'Use "dexscreener watchlist add <token>" to add tokens',
            "info"
          );
          return;
        }
    ...
    window.terminal.displayWatchlistItem = function (pair, index) {
      ...
      html += `<div>`;
      html += `<div style='font-size:1.1em;font-weight:bold;color:#00ccff;'>${
        pair.baseToken?.symbol || "N/A"
      }</div>`;
      ...
      if (pair.priceChange?.h24 !== undefined) {
        const changeColor = pair.priceChange.h24 >= 0 ? "#00ff00" : "#ff3333";
        const changePrefix = pair.priceChange.h24 >= 0 ? "+" : "";
        html += `<div style='color:${changeColor};font-size:0.9em;'>${changePrefix}${pair.priceChange.h24.toFixed(
          2
        )}%</div>`;
      }
      html += `</div></div>`;
      if (pair.volume?.h24) {
        html += `<div style='color:#666;font-size:0.8em;margin-top:6px;'>Vol: $${this.formatNumber(
          pair.volume.h24
        )}</div>`;
      }
      html += `</div>`;
    ...
    window.terminal.removeFromPortfolio = function (tokenSymbol) {
      try {
        ...
        this.log(
          `❌ Failed to remove from portfolio: ${error.message}`,
          "error"
        );
      }
    };
        ...
        for (let i = 0; i < Math.min(data.length, 8); i++) {
          ...
          try {
            ...
            const pairResponse = await fetch(
              `https://api.dexscreener.com/latest/dex/search?q=${profile.tokenAddress}`
            );
            ...
            this.log(
              `⚠️ Failed to get details for ${profile.tokenAddress}: ${error.message}`,
              "error"
            );
            this.displayBasicTrendingCard(profile, i + 1);
          }
    ...
    window.terminal.displayBasicTrendingCard = function (profile, index) {
      ...
      html += `<div>`;
      html += `<div style='font-size:1.1em;font-weight:bold;color:#ff6666;'>${profile.tokenAddress.slice(
        0,
        8
      )}...</div>`;
      ...
      if (profile.description) {
        html += `<div style='color:#bbb;font-size:0.8em;margin-top:6px;'>${profile.description.slice(
          0,
          100
        )}...</div>`;
      }
      ...
      try {
        ...
        for (let i = 0; i < Math.min(data.length, 8); i++) {
          ...
          try {
            ...
            this.log(
              `⚠️ Failed to get details for ${boost.tokenAddress}: ${error.message}`,
              "error"
            );
            this.displayBasicBoostedCard(boost, i + 1);
          }
        ...
        this.log(
          `❌ Boosted tokens analysis failed: ${error.message}`,
          "error"
        );
      }
    };
    ...
    window.terminal.displayBoostedTokenCard = function (pair, boost, index) {
      ...
      if (boost.icon) {
        ...
        html += `<div style='width:36px;height:36px;border-radius:50%;background:#ffa500;color:#000;display:flex;align-items:center;justify-content:center;font-weight:bold;'>${symbol.charAt(
          0
        )}</div>`;
      }

      html += `<div>`;
      html += `<div style='font-size:1.3em;font-weight:bold;color:#ffa500;'>${
        pair.baseToken?.symbol || "N/A"
      }</div>`;
      html += `<div style='color:#888;font-size:0.85em;'>${pair.chainId.toUpperCase()} • ${
        pair.dexId || "Unknown DEX"
      }</div>`;
      html += `</div>`;
      html += `</div>`;
      ...
      if (boost.amount || boost.totalAmount) {
        html += `<div style='color:#ffa500;font-size:0.9em;font-weight:bold;'>🚀 ${
          boost.totalAmount || boost.amount || "N/A"
        } boost(s)</div>`;
      }
      html += `</div></div>`;
      ...
      if (boost.description) {
        html += `<div style='color:#bbb;font-size:0.9em;margin-bottom:10px;line-height:1.3;'>${boost.description.slice(
          0,
          120
        )}...</div>`;
      }
      ...
      if (pair.volume?.h24) {
        ...
        html += `<div style='color:#fff;'>$${this.formatNumber(
          pair.volume.h24
        )}</div>`;
        html += `</div>`;
      }
      ...
      if (pair.marketCap || pair.fdv) {
        ...
        html += `<div style='color:#ff6666;font-weight:bold;'>💎 ${
          pair.marketCap ? "MCap" : "FDV"
        }</div>`;
        html += `<div style='color:#fff;'>$${this.formatNumber(
          capValue
        )}</div>`;
      ...
      html += `<button onclick="window.terminal.addToWatchlist('${
        pair.baseToken.address || pair.pairAddress
      }')" style='background:#ffa500;color:#000;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;font-size:0.8em;font-weight:bold;'>📋 Add to Watchlist</button>`;
      html += `<button onclick="window.open('https://dexscreener.com/${pair.chainId}/${pair.pairAddress}', '_blank')" style='background:#0088ff;color:#fff;border:none;padding:6px 12px;border-radius:4px;cursor:pointer;font-size:0.8em;font-weight:bold;'>🔗 View on DexScreener</button>`;
      html += `</div>`;
    ...
    window.terminal.displayBasicBoostedCard = function (boost, index) {
      ...
      html += `<div>`;
      html += `<div style='font-size:1.1em;font-weight:bold;color:#ffa500;'>${boost.tokenAddress.slice(
        0,
        8
      )}...</div>`;
      ...
      if (boost.description) {
        html += `<div style='color:#bbb;font-size:0.8em;margin-top:6px;'>${boost.description.slice(
          0,
          100
        )}...</div>`;
      }
    ...
    window.terminal.isNewGem = function (pair, criteria) {
      ...
      if (
        criteria.minVolume &&
        (!pair.volume?.h24 || pair.volume.h24 < criteria.minVolume)
      ) {
        return false;
      }
      ...
      if (
        criteria.minLiquidity &&
        (!pair.liquidity?.usd || pair.liquidity.usd < criteria.minLiquidity)
      ) {
        return false;
      }
      ...
      if (
        criteria.minPriceChange &&
        (!pair.priceChange?.h24 ||
          pair.priceChange.h24 < criteria.minPriceChange)
      ) {
        return false;
      }
    ...
    window.terminal.displayGemCard = function (gem, index) {
      ...
      if (gem.info?.imageUrl || gem.profile?.icon) {
        ...
        html += `<div style='width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#ffd700,#ffb347);color:#000;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:1.2em;border:3px solid #ffd700;box-shadow:0 2px 8px rgba(255,215,0,0.4);'>${symbol.charAt(
          0
        )}</div>`;
      }

      html += `<div>`;
      html += `<div style='font-size:1.4em;font-weight:bold;color:#ffd700;text-shadow:0 2px 4px rgba(255,215,0,0.3);'>${
        gem.baseToken?.symbol || "NEW GEM"
      }</div>`;
      html += `<div style='color:#ffb347;font-size:0.95em;'>${
        gem.baseToken?.name || "Unknown Token"
      }</div>`;
      html += `<div style='color:#888;font-size:0.85em;'>${gem.chainId.toUpperCase()} • ${
        gem.dexId || "Unknown DEX"
      }</div>`;
      html += `</div></div>`;
      ...
      if (gem.priceUsd) {
        ...
        if (gem.priceChange?.h24 !== undefined) {
          ...
          html += `<div style='color:${changeColor};font-size:1.2em;font-weight:bold;text-shadow:0 2px 4px rgba(${
            gem.priceChange.h24 >= 0 ? "0,255,0" : "255,51,51"
          },0.3);'>${changePrefix}${gem.priceChange.h24.toFixed(
            2
          )}% (24h)</div>`;
        }
        html += `</div>`;
      ...
      if (gem.volume?.h24) {
        ...
        html += `<div style='color:#fff;font-weight:bold;font-size:0.9em;'>$${this.formatNumber(
          gem.volume.h24
        )}</div>`;
        html += `</div>`;
      } else {
      ...
      if (gem.pairCreatedAt) {
        ...
        const ageColor =
          ageInDays <= 1 ? "#00ff00" : ageInDays <= 3 ? "#ffcc00" : "#ff8800";
        html += `<div style='text-align:center;padding:10px;background:rgba(${
          ageInDays <= 1
            ? "0,255,0"
            : ageInDays <= 3
            ? "255,204,0"
            : "255,136,0"
        },0.12);border-radius:8px;border:1px solid ${ageColor};'>`;
      ...
      if (gem.pairCreatedAt) {
        const ageInDays = Math.floor(
          (Date.now() - gem.pairCreatedAt * 1000) / (1000 * 60 * 60 * 24)
        );
        if (ageInDays <= 7)
          reasons.push(`🆕 Very new token (${ageInDays}d old)`);
      }
      if (gem.priceChange?.h24 && gem.priceChange.h24 > 10) {
        reasons.push(
          `📈 Strong price momentum (+${gem.priceChange.h24.toFixed(1)}%)`
        );
      }
      if (gem.volume?.h24 && gem.volume.h24 > 50000) {
        reasons.push(
          `🔥 High trading activity ($${this.formatNumber(gem.volume.h24)} vol)`
        );
      }
      if (gem.liquidity?.usd && gem.liquidity.usd > 25000) {
        reasons.push(
          `💧 Good liquidity ($${this.formatNumber(gem.liquidity.usd)})`
        );
      }
      if (reasons.length === 0) {
      ...
      html += `<button onclick="window.terminal.showTokenAnalytics('${
        gem.baseToken.address || gem.pairAddress
      }', '${
        gem.baseToken.symbol || "GEM"
      }')" style='background:linear-gradient(135deg,#ff1493,#ff69b4);color:#fff;border:none;padding:8px 14px;border-radius:6px;cursor:pointer;font-size:0.85em;font-weight:bold;box-shadow:0 3px 8px rgba(255,20,147,0.4);'>📊 Deep Analysis</button>`;
    ...
    try {
      if (
        typeof window.terminal.log === "function" &&
        typeof window.terminal.logHtml === "function"
      ) {
        console.log(
          "✅ DexScreener Analytics Ultimate Plugin v2.0 loaded successfully!"
        );
        console.log(
          'ℹ️ Use "window.terminal.dexAnalyticsRestore()" if you experience any issues'
        );
      } else {
      ...
      console.error(
        "❌ DexScreener Analytics Plugin initialization failed:",
        error
      );
